...
int main() {
    try {
        ...
        io::CSVReader<4, io::trim_chars<' ', '\t'>, io::double_quote_escape<',','\"'> > cleanassigneeReader("c:\\Users\\marsh\\Patent Tracking\\Sorted\\Sorted_assignee\\Sorted_Cleaned_assignee_assignee_cleaned.csv");
        cleanassigneeReader.read_header(io::ignore_extra_column, "patnum", "assignee", "pgpub_id", "assignee_cleaned");


        std::vector<std::vector<std::string>> cleanassignees; // Remove this line, it's a duplicate declaration
        std::string patnum, uncleanedAssignee, pgpubId, cleanedAssignee;
        ...
        // Read the fourth input file for CSRP permco dataset sorted by cleaned company name
        std::vector<std::vector<std::string>> cleandata;
        io::CSVReader<3, io::trim_chars<' ', '\t'>, io::no_quote_escape<','> > cleandataReader("c:\\Users\\marsh\\Patent Tracking\\Sorted\\Sorted_crsp_permo\\CSRP_Permco_Sorted_by_UnCleaned.csv");
        std::string uncleanedCompanyName, companyPatnum, cleanedCompanyName;
        while(cleandataReader.read_row(uncleanedCompanyName, companyPatnum, cleanedCompanyName)){
            cleandata.push_back({uncleanedCompanyName, companyPatnum, cleanedCompanyName});
        }
        ...
        //For matching cleaned assignees with clean CSRP permco dataset
        for (size_t i = 0; i < assignees.size(); ++i) {
            const auto& assigneeRow = cleanassignees[i];
            std::string cleanassignee = assigneeRow[1]; // Using Uncleaned Assignee for matching
            int searchIndex = binary_search(data, 0, assignee);
            int levenshteinScore = -1;
            int lengthScoreVal = -1;
            std::vector<std::string> row;
